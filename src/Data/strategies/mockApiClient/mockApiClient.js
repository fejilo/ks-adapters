...
export default {
  ...
  async userUpdate(
    { firstName, lastName, imageFileId, defaultCampaignModeId, writeNulls = 0 },
    token
  ) {
    await new Promise((resolve) => setTimeout(resolve, 1000));
    if (!token) throw new Error("Invalid token");
  ...
  async userPasswordUpdate(
    { username, passwordOld, passwordNew, userRecoveryUuid },
    token
  ) {
    await new Promise((resolve) => setTimeout(resolve, 1000));
    if (passwordNew && userRecoveryUuid) return userPasswordUpdate;
    if ((username && passwordOld && passwordNew, token))
  ...
  async userCampaignModeCreate({ campaignModeName, campaignIdCsv }, token) {
    await new Promise((resolve) => setTimeout(resolve, 1000));
    if (!campaignModeName || !campaignIdCsv)
      throw new Error("Invalid parameters");
    if (!token) throw new Error("Invalid token");
    return {
